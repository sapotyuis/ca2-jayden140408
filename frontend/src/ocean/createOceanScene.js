...
/**
 * Framework-agnostic Three.js night-ocean scene. It knows nothing about React — the page
 * component passes in a canvas, an authenticated `api` function, and a set of callbacks it uses
 * to push HUD updates back out (status, catch log, POV changes...). This keeps the imperative
 * 3D loop (which should never run through React's reconciler) cleanly separated from the
 * declarative React HUD around it, and returns a handle with { togglePov, dispose } so the
...
export function createOceanScene({
  ...
  const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth / 3, window.innerHeight / 3), 0.85, 0.55, 0.28);
  ...
  const updateWorldLighting = () => {
    ...
    bloomPass.strength = THREE.MathUtils.lerp(0.9, 0.42, transition);
  ...
  const createSurvivor = () => {
    ...
    survivorEye = new THREE.Object3D();
    survivorEye.name = 'survivor-eye-anchor';
    survivorEye.position.set(0, 1.52, -0.04);
    survivor.add(survivorEye);
    return survivor;
  };
  ...
  const firstPersonEye = new THREE.Vector3();
  const updateCamera = (snap = false, elapsed = 0) => {
    ...
    if (povMode === 'third') {
      ...
      if (survivorEye) {
        survivorEye.getWorldPosition(firstPersonEye);
      } else {
        firstPersonEye.copy(raftState.position).add(new THREE.Vector3(0.55, 1.9, 0.28));
      }
      camera.position.copy(firstPersonEye);
      const target = getFirstPersonLookTarget(firstPersonEye, heading, lookYaw, lookPitch);
      camTarget.set(target.x, target.y, target.z);
      camera.lookAt(camTarget);
    }
  };

  function togglePov() {
    povMode = povMode === 'third' ? 'first' : 'third';
    onPovChange?.(povMode);
  }
  ...
  const createDebrisAsset = (itemName) => {
    ...
    if (name.includes('wood') || name.includes('plank')) {
      ...
      const fish = debrisMaterial(0xc67a3a);
      const belly = debrisMaterial(0xe1b276);
      const fin = debrisMaterial(0x8c4b2d);
      const eye = debrisMaterial(0x0a1115, { roughness: 0.35 });
      addDebrisMesh(group, new THREE.SphereGeometry(0.36, 14, 10), fish, [-0.08, 0.38, 0], [0, 0, 0], [1.55, 0.72, 0.72]);
      addDebrisMesh(group, new THREE.SphereGeometry(0.22, 12, 8), belly, [-0.22, 0.27, 0.18], [0, 0, 0], [1.3, 0.35, 0.55]);
      addDebrisMesh(group, new THREE.ConeGeometry(0.32, 0.62, 3), fin, [0.92, 0.38, 0], [0, 0, Math.PI / 2]);
      addDebrisMesh(group, new THREE.ConeGeometry(0.13, 0.36, 3), fin, [-0.08, 0.72, 0], [0, 0, Math.PI]);
      addDebrisMesh(group, new THREE.SphereGeometry(0.055, 8, 8), eye, [-0.56, 0.48, 0.23]);
    } else if (name.includes('coconut')) {
      const husk = debrisMaterial(0x4e2d1b);
      const top = debrisMaterial(0x1c130d);
      addDebrisMesh(group, new THREE.SphereGeometry(0.48, 14, 10), husk, [0, 0.42, 0], [0.15, 0.35, 0], [1, 0.9, 0.95]);
      addDebrisMesh(group, new THREE.TorusGeometry(0.12, 0.035, 6, 12), top, [0.08, 0.82, 0.02], [Math.PI / 2, 0, 0]);
      for (const angle of [0.3, 1.9, 3.4, 5.1]) {
        addDebrisMesh(group, new THREE.CylinderGeometry(0.018, 0.03, 0.55, 5), top, [Math.cos(angle) * 0.25, 0.42, Math.sin(angle) * 0.25], [Math.sin(angle) * 0.65, 0, Math.cos(angle) * 0.65]);
      }
      ...
      const box = debrisMaterial(0xb34b36, { roughness: 0.7 });
      const label = debrisMaterial(0xf0d28b, { roughness: 0.85 });
      addDebrisMesh(group, new THREE.BoxGeometry(0.88, 0.48, 0.62), box, [0, 0.3, 0], [0, 0.2, 0]);
      addDebrisMesh(group, new THREE.BoxGeometry(0.04, 0.49, 0.64), label, [0, 0.3, 0], [0, 0.2, 0]);
  ...
  const boot = async () => {
    ...
    if (status.ok) onStatus?.({ materials: status.data.materials, hunger: status.data.hunger, raftSize: status.data.raft_size });
    if (collectiblesEnabled) await loadCollectibles();
    await loadUnexpectedEvents();
    updateOcean(0);
    updateWorldLighting();
    updateCamera(true, 0);
    composer.render();
  ...
  return { togglePov, triggerDemoEvent, dispose };
}
