...
import { button, statChip, worldClock } from '../components/vanilla';
import { createGameState } from '../lib/gameState';
import { craftingPanel, inventoryPanel, profilePanel, questsPanel, statusPanel, upgradesPanel } from '../components/game/vanillaPanels';
...
export const renderGamePage = ({ root, auth, toast }) => {
  ...
  const onClick = async (event) => {
    ...
    if (action === 'voyage') return window.location.assign('/voyage.html');
    if (action === 'leaderboard') return window.location.assign('/leaderboard.html');
    if (action === 'logout') { auth.logout(); return window.location.replace('/login.html'); }
