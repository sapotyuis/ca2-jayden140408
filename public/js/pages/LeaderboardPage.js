...
import { LEADERBOARD_LIMIT, rankSurvivors } from '../lib/leaderboard.js';
import { escapeHtml, formatNumber } from '../lib/dom.js';
import { mountOceanViewport } from '../components/oceanViewport.js';
import { worldClock } from '../components/vanilla.js';
...
export const renderLeaderboardPage = ({ root, auth, worldClock: worldClockStore }) => {
  ...
  root.innerHTML = `<div class="${styles.shell}"><div data-leaderboard-world class="${styles.world}"></div><main class="${styles.page}">
    <header class="${styles.header}"><div><span class="${styles.kicker}">CASTAWAY CHRONICLES / PUBLIC ROSTER</span><p class="${styles.brand}">SURVIVOR LEDGER</p></div><div class="${styles.actions}">${worldClock(worldClockStore.getState(), true)}<a href="${auth.getState().isAuthed ? '/html/camp.html' : '/html/login.html'}" class="${styles.backLink}">${auth.getState().isAuthed ? 'BACK TO CAMP' : 'BACK TO LOGIN'}</a></div></header>
    ...
    <footer class="${styles.footer}"><span>TOP FIVE PUBLIC ROSTER</span><a href="/html/register.html">CREATE YOUR SURVIVOR ACCOUNT</a></footer>
  </main></div>`;
