...
import { button, statChip, worldClock } from '../components/vanilla.js';
import { createGameState } from '../lib/gameState.js';
import { craftingPanel, inventoryPanel, profilePanel, questsPanel, statusPanel, upgradesPanel } from '../components/game/vanillaPanels.js';
...
export const renderGamePage = ({ root, auth, toast }) => {
  ...
  const onClick = async (event) => {
    ...
    if (action === 'voyage') return window.location.assign('/html/voyage.html');
    if (action === 'leaderboard') return window.location.assign('/html/leaderboard.html');
    if (action === 'logout') { auth.logout(); return window.location.replace('/html/login.html'); }
