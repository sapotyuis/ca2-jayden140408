// Renders the public leaderboard, handles loading/errors, and displays ranked survivor data.
import { createClassNames } from '../helpers/cssClassNames.js';
import { LEADERBOARD_LIMIT, rankSurvivors } from '../game/leaderboard.js';
import { escapeHtml, formatNumber } from '../helpers/domHelpers.js';
import { mountOceanViewport } from '../voyage/oceanCanvas.js';
import { worldClock } from '../helpers/uiComponents.js';

const styles = createClassNames('leaderboard-page');
...
export const renderLeaderboardPage = ({ root, auth, worldClock: worldClockStore }) => {
  root.innerHTML = `<div class="${styles.shell}"><div data-leaderboard-world class="${styles.world}"></div><main class="${styles.page}">
    <header class="${styles.header}"><div><span class="${styles.kicker}">CASTAWAY CHRONICLES / PUBLIC ROSTER</span><p class="${styles.brand}">SURVIVOR LEDGER</p></div><div class="${styles.actions}">${worldClock(worldClockStore.getState())}<a href="${auth.getState().isAuthed ? '/camp' : '/login'}" class="${styles.backLink}">${auth.getState().isAuthed ? 'BACK TO CAMP' : 'BACK TO LOGIN'}</a></div></header>
    ...
    <footer class="${styles.footer}"><span>TOP FIVE PUBLIC ROSTER</span><a href="/register">CREATE YOUR SURVIVOR ACCOUNT</a></footer>
  </main></div>`;
  const world = root.querySelector('[data-leaderboard-world]');
  const content = root.querySelector('[data-leaderboard-content]');
  mountOceanViewport(world, { auth, worldClock: worldClockStore, mode: 'title', interactive: false, collectiblesEnabled: false, fetchStatus: false });
