// Renders the voyage HUD, displays collection and event feedback, and connects the page to the ocean world.
...
import { pixelIcon, worldClock } from '../helpers/uiComponents.js';

const styles = createClassNames('ocean-page');
...
export const renderOceanPage = ({ root, auth, worldClock: worldClockStore }) => {
  ...
  const scene = mountOceanViewport(canvasHost, {
    ...
    onUnexpectedEvent: (event) => { eventAlert = event; window.clearTimeout(alertTimer); alertTimer = window.setTimeout(() => { eventAlert = null; render(); }, 7600); render(); },
    onReady: () => { loading = false; render(); },
  });
  const onClick = (event) => {
    const back = event.target.closest('[data-action="back"]');
    if (back) return window.location.assign('/camp');
    const demo = event.target.closest('[data-demo]');
    if (demo) scene.triggerDemoEvent(demo.dataset.demo);
  };
